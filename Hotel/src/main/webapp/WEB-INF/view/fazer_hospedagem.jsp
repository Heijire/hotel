...
<head>
    ...
    <title>Realizar Hospedagem</title>
</head>
<body>
    ...
    <div>   <!--div de controle de dados-->
        <div> <!-- div da tabela de input-->
            <form action="fazer_hospedagem" method="post">
                <h1>Realizar Hospedagem</h1>
                <table>
                    <input type="hidden" name="id" value="${hospedagem.id}">
                    <tr>
                        <td><label for="cpf">CPF do Cliente:</label></td>
                        <td><input type="number" id="cpf" name="cpf" value='<c:out value="${hospedagem.cliente.cpf}"/>'></td>
                    </tr>
                    <tr>
                        <td><label for="numero">Número do Quarto:</label></td>
                        <td><input type="number" id="numero" name="numero" value='<c:out value="${hospedagem.quarto.numero}"/>'></td>
                    </tr>
                    <tr>
                        <td><label for="dt_entrada">Data de entrada:</label></td>
                        <td><input type="date" name="dt_entrada" id="dt_entrada" value='<c:out value="${hospedagem.dtEntrada}"/>'></td>
                    </tr>
                    <tr>
                        <td><label for="qntDias">Quantidade de Dias:</label></td>
                        <td><input type="number" name="qntDias" id="qntDias" value='<c:out value="${hospedagem.qntDias}"/>'></td>
                    </tr>
        ...
        <div> <!--div da tabela de apresentacao de dados-->
            <c:if test="${not empty hospedagens}">
                <table>
                    ...
                    <tbody>
                        <c:forEach var="hospedagens" items="${hospedagens}">
                            <tr>
                                <td>${hospedagens.cliente.cpf}</td>
                                <td>${hospedagens.quarto.numero}</td>
                                <td>${hospedagens.dtEntrada}</td>
                                <td>${hospedagens.qntDias}</td>
                                
                                <td>
                                    <a class="a_link_clicavel"
									href="${pageContext.request.contextPath}/fazer_hospedagem?acao=editar&id=${hospedagem.id}">Editar</a>
								</td>
									
								<td>
                                    <a class="a_link_clicavel"
									href="${pageContext.request.contextPath }/fazer_hospedagem?acao=excluir&id=${hospedagem.id}">Deletar</a>
                                </td>
